...
// --- NEW: Ask the server if the password is correct ---
function checkPassword() {
  const pwd = passwordInput.value.trim();
  socket.emit('verifyPassword', pwd); // Send attempt to the server
}

// --- NEW: Listen for the server's response ---
socket.on('passwordResult', (isValid) => {
  if (isValid) {
    passwordScreen.style.display = 'none';
    initCinema(); // Only initialize if server says YES
  } else {
...
} // <-- Bracket fixed: properly closes initCinema here

// Start with password screen focused
passwordInput.focus();
